...
1. Open **Projects** and use **Sign in** at the top right to sign in with Google. Once signed in, your avatar/name opens the profile menu with your sharing code, settings and sign-out. **Account projects** and **On this device** are separate project-list tabs.
...
3. To share, select **Share** on a project you own. Type at least two characters of the person's Google display name and select their account, or paste their sharing code into the same field. Results show their photo and the end of their code to distinguish similar names. They must have signed in on the same server first; codes remain available under **Copy my sharing code** in the profile menu. Choose **Can edit** or **Can view**, then **Share**. The dialog confirms the saved member and offers **Copy project link**; a link alone does not grant access. The same dialog can change or remove access. The recipient's account list refreshes when returning to the page and every 15 seconds while visible, with a refresh button beside **Import project** too. Local-device projects and search filters remain separate. See [server diagnostics](logging.md) to investigate missing projects or failed sharing.
...
5. On account boards, **Save → Download this project** exports one project. **Import project** beside the gallery's storage tabs restores it as an account project. Old full-workspace backups must first be restored through a local board's existing Save menu, then imported project by project.
...
When browser storage is available, unsaved edits retain a recovery draft with their base document. Reopening automatically merges compatible drafts onto the current shared board and resumes saving without a restore dialog. Older drafts without a matching base, unreadable drafts and drafts belonging to viewers are preserved separately; the current shared board opens normally, with **Save → Download older unsaved edits** available. Browser storage being unavailable does not prevent server use. Navigation first attempts saving, then can leave silently with a recovery draft or a matching download. If neither is available during a failed save, in-app navigation stays on the canvas with a toolbar hint. Saving failures never open automatic dialogs, including browser-close prompts. Closing the browser while both server saving and draft storage are unavailable can therefore lose edits; the toolbar offers saving details and a download.

Incoming live edits apply even while an HTTP save response is pending. Late acknowledgments cannot rewind newer revisions, and edits made after sending are retained. Mixed text/movement updates reuse unaffected elements and animate remote geometry; drawings retain their paths during unrelated changes. Remote deletion cleanly ends a gesture targeting that object. Run `node tests/run-database-tests.mjs --canvas` for the focused concurrent-action, delayed-save, drawing/deletion and silent-recovery browser scenarios.
...
Each ordinary action starts saving immediately, including typing and each arrow-key movement before key release. Active dragging/resizing/drawing is sampled every 100 ms and saves its final state on release. Each snapshot is also kept in the browser's recovery draft until acknowledged. One request runs at a time; changes made during it are combined into the next save. Server broadcasts coalesce for 20 ms. Actual delivery depends on network and database latency; offline changes retry when access returns. Remote movement interpolates display geometry while retaining exact saved coordinates, and reuses existing canvas and cursor elements. Database polling also catches edits from older clients or another backend within about a second. Every write is durable before acknowledgment. An unchanged retry is a no-op; a later field write, including a retry, replaces that field. Late field edits to deleted objects are ignored, and retried creations preserve existing objects. Existing whole-document PUT clients keep strict revision checks.
